...
from pyramid.httpexceptions import (HTTPException, HTTPFound, HTTPNotFound, HTTPBadRequest, HTTPConflict, HTTPInternalServerError)
...
def logUsage(event):
	...
	ignoreList = ['/css','/js','/images','/media','/favicon']  # ignore if requestString starts with any of these strings
...
# ---------------------------------------------------------
@view_config(route_name='/home', renderer="haemosphere:templates/home.mako")
...
def searchPage(request):
	...
	for ds in allDatasets:
		sampleGroups[ds['name']] = [{'name':group, 'items':ds['sampleGroupItems'][group]} for group in ds['sampleGroups']]
			
	return {"datasets":allDatasets, "sampleGroups":sampleGroups, "selectedSearch":selectedSearch, 'topTableString':rfunctions.topTableString}
...
def searchKeyword(request):
	...
	searchTerms = filter(None, [ t.strip() for t in re.split('[,\s\n\r]+', searchString) ])
	if not searchTerms:
	...
	gs = hsgeneset.HSGeneset(name='Search: %s' % genesetName, description=description) \
				.subset(queryStrings=searchTerms, searchColumns=searchColumns, species=species, caseSensitive=exactMatch, matchSubstring= not exactMatch)
...
def searchExpression(request):
	...
	if len(replicateSampleGroupItems)==len(set(replicateSampleGroupItems)) or len(sampleTable1)<2 or len(sampleTable2)<2:
		return {'error':"You have chosen a dataset and sample group where there is not enough replicates for a statistical analysis"}
	...
	topTable = rfunctions.topTable(df, ds.sampleGroupItems(sampleGroup=ds.replicateSampleGroup(), duplicates=True), ds.sampleGroupItems(sampleGroup=sampleGroup, duplicates=True), \
		sampleGroupItem1, sampleGroupItem2, not ds.isRnaSeqData, filterMinCpm=filterMinCpm, filterMinExpressedSamples=minFilterSample, normalizationMethod=normalization,
	...
	description = 'Geneset from expression search, with dataset %s, %s vs %s' % (ds.name, sampleGroupItem1, sampleGroupItem2)
	gs = hsgeneset.HSGeneset(name='Diff Exp: %s' % genesetName, description = description).subset(queryStrings=geneIds, searchColumns=['GeneId'])
...
def searchHighExpression(request):
	...
	scores = dict([(rowId,value) for rowId,value in scores.iteritems() if value>0])   # only keep positive scores
	...
	if not ds.isRnaSeqData:	# fetch matching geneIds and assign topTable values to them
		gpid = ds.geneIdsFromProbeIds(probeIds=scores.keys(), returnType='dict')
		newScores = {}
		for probeId,value in scores.iteritems():
			if probeId not in gpid: continue
	...
	gs = hsgeneset.HSGeneset(name='High Exp: %s' % genesetName, description = description).subset(queryStrings=geneIds, searchColumns=['GeneId'])
