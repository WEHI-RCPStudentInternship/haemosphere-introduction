	...
	if not matchingDatasets:
		...
		if not error:	# looks OK to plot, so supply all necessary variables
			...
			if ds.isRnaSeqData:  # take log of values
				logValues = {}
				for geneId,vals in expressionValues.iteritems():
					logValues[geneId] = dict([(sampleId, numpy.log2(val+1)) for sampleId,val in vals.iteritems()])
...
def showExpressionMultiSpecies(request):
	...
	if geneId not in df.index:
		...
		for species,items in datasetsToUse.iteritems():
		...
		for species,items in filteredDatasetsToUse.iteritems():
			for item in items:
...
def showExpressionGeneVsGene(request):
	...
	if "celltype" not in ds.sampleGroups(returnType="display") or "cell_lineage" not in ds.sampleGroups(returnType="display"):
		...
		for lineage,celltypes in ds.sampleGroupItems(sampleGroup="celltype", groupBy="cell_lineage").iteritems():
